      ...
      for (let inde = 0; inde < fF[ind].length; inde++) {
        ...
        for (let ti = 0; ti < fF[ind][inde].WantedMBTI.length; ti++) {
          for (let i = 0; i < MBTI[fF[ind][inde].WantedMBTI[ti]].length; i++) {
            if (
              fF[ind][inde].STnumber !=
              MBTI[fF[ind][inde].WantedMBTI[ti]][i].STnumber
            ) {
        ...
        // 3. 동성 매칭 가능 여부 판단
        if (!fF[ind][inde].SameGender) {
          // 이성
          index = 0;
          for (let ti = 0; ti < temp.length; ti++) {
            if (fF[ind][inde].Gender == temp[ti].Gender) {
              // 동성인 경우
              temp.splice(index, 1);
              index--;
            } else if (temp[ti].SameGender) {
        ...
        for (let ti = 0; ti < temp.length; ti++) {
          ...
          for (let i = 0; i < temp[ti].WantedMBTI.length; i++) {
            if (fF[ind][inde].OwnMBTI == temp[ti].WantedMBTI[i]) {
              score++;
            }
          }
      ...
      for (let inde = 0; inde < cF[ind].length; inde++) {
        ...
        for (let ti = 0; ti < cF[ind][inde].WantedMBTI.length; ti++) {
          for (let i = 0; i < MBTI[cF[ind][inde].WantedMBTI[ti]].length; i++) {
            if (
              cF[ind][inde].STnumber !=
              MBTI[cF[ind][inde].WantedMBTI[ti]][i].STnumber
            ) {
        ...
        // 3. 동성 매칭 가능 여부 판단
        if (!cF[ind][inde].SameGender) {
          // 이성
          index = 0;
          for (let ti = 0; ti < temp.length; ti++) {
            if (cF[ind][inde].Gender == temp[ti].Gender) {
              // 동성인 경우
              temp.splice(index, 1);
              index--;
            } else if (temp[ti].SameGender) {
        ...
        for (let ti = 0; ti < temp.length; ti++) {
          ...
          // 4. 쌍방 선호 MBTI
          for (let i = 0; i < temp[ti].WantedMBTI.length; i++) {
            if (cF[ind][inde].OwnMBTI == temp[ti].WantedMBTI[i]) {
              score++;
            }
          }
      ...
      for (let inde = 0; inde < eF[ind].length; inde++) {
        ...
        for (let ti = 0; ti < eF[ind][inde].WantedMBTI.length; ti++) {
          for (let i = 0; i < MBTI[eF[ind][inde].WantedMBTI[ti]].length; i++) {
            if (
              eF[ind][inde].STnumber !=
              MBTI[eF[ind][inde].WantedMBTI[ti]][i].STnumber
            ) {
        ...
        // 3. 동성 매칭 가능 여부 판단
        if (!eF[ind][inde].SameGender) {
          // 이성
          index = 0;
          for (let ti = 0; ti < temp.length; ti++) {
            if (eF[ind][inde].Gender == temp[ti].Gender) {
              // 동성인 경우
              temp.splice(index, 1);
              index--;
            } else if (temp[ti].SameGender) {
        ...
        for (let ti = 0; ti < temp.length; ti++) {
          ...
          // 4. 쌍방 선호 MBTI
          for (let i = 0; i < temp[ti].WantedMBTI.length; i++) {
            if (eF[ind][inde].OwnMBTI == temp[ti].WantedMBTI[i]) {
              score++;
            }
          }
  ...
  try {
    while (1) {
      ...
      for (let i = 0; i < infos.length; i++) {
        ...
        } // 매칭이 성사될 수 없는 사람을 제외하고 모든 사람의 매칭이 끝나면 종료
      }

      let max = 0;
      for (let j = 1; j < infos[index].MatchedPeople.length; j++) {
        // 점수가 가장 높은 후보를 선택
        if (infos[index].MatchedScore[max] < infos[index].MatchedScore[j])
          max = j;
      }
      ...
      for (let i = 1; i < updateInfos.length; i++) {
        // 매칭이 완료된 사람들은 다른 사람들의 후보 리스트에서 제외
        index = 0;
        for (let j = 0; j < updateInfos[i].MatchedPeople.length; j++) {
          if (
            updateInfos[i].MatchedPeople[index] == a ||
            infos[i].MatchedPeople[index] == b
          ) {
        ...
        await StudentInfo.updateOne(
          { STnumber: updateInfos[i].STnumber },
          {
            MatchedPeople: updateInfos[i].MatchedPeople,
            MatchedScore: updateInfos[i].MatchedScore,
          }
        );
      }
    }
